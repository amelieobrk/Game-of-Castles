...
public class Resources {
    ...
    private void saveScoreEntries() throws IOException {
    	...
    	PrintWriter out = new PrintWriter(
    			new OutputStreamWriter(new FileOutputStream("highscores.txt"), "UTF-8"));
    	
    	scoreEntries.get(0).write(out);
    	for (int i = 1; i < scoreEntries.size(); i++) {
    		out.write("/n");
    		scoreEntries.get(i).write(out);
    	}
    }
    ...
    private void loadScoreEntries() {
    	...
    	scoreEntries =  new LinkedList<ScoreEntry>();
    	try {
    		String[] entries = this.readLinesFromFile("highscores.txt");
    		for (int i = 0; i < entries.length; i++) {
    			scoreEntries.add(ScoreEntry.read(entries[i]));
    		}
    		Collections.sort(scoreEntries);
    	} catch (FileNotFoundException f){
    		System.out.println("Fehler beim Laden der Datein: " + f.getMessage());
    	} catch (UnsupportedEncodingException u) {
    		System.out.println("Fehler beim Laden der Datein: " + u.getMessage());
    	}
    }
    
    private String[] readLinesFromFile(String path) throws FileNotFoundException, UnsupportedEncodingException{
    	BufferedReader bR = new BufferedReader(
    			new InputStreamReader(new FileInputStream(path), "UTF-8"));
    	return bR.lines().toArray(s -> new String[s]);
    }
